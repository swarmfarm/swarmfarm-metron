...
extension Polygon {
    ...
    public var isSelfIntersecting: Bool {
        ...
        while let segment = lineSegments.popLast() {
            if let _ = lineSegments.first(where: {
                // Check concurrent points - if only one is concurrent this can be skipped
                let anyPointsConcurrent = segment.a == $0.a || segment.a == $0.b || segment.b == $0.a || segment.b == $0.b
                if anyPointsConcurrent {
                    let sameConcurrent = segment.a == $0.a && segment.b == $0.b
                    let sameInvertedConcurrent = segment.a == $0.b && segment.b == $0.a
                    
                    return sameConcurrent || sameInvertedConcurrent
                }
                else {
                    return $0.intersection(with: segment) != nil
                }
            }) {
                return true
            }
